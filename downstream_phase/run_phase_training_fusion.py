...
from downstream_phase.engine_for_phase_fusion import (
    train_one_epoch,
...
def get_args():
    ...
    parser.add_argument(
        "--pretrained_path",
        default="/data/gsw/Code/Surgformer/pretrain_params/TimeSformer_divST_8x32_224_K400.pyth",
        type=str,
        metavar="Parameters",
        help="Name of parameters to load",
    )
    parser.add_argument(
        "--pretrained_path_of_fusion",
        default="/data/gsw/Code/Surgformer/imagenet21k+imagenet2012_ViT-B_16-224.npz",
        type=str,
        metavar="Parameters",
        help="Name of parameters to load",
    )
...
def main(args, ds_init):
    ...
    args.output_dir = os.path.join(
        ...
        "_".join(
            [
                ...
                frame_manner,
                "withfalphaweight-(1-alpha)-0.5alpha-0.5alpha"
            ]
        ),
    )
    ...
    rgbmodel.load_from(np.load(args.pretrained_path_of_fusion))

    fusionmodel = LSTA(num_classes=args.nb_classes)
    combinedModel = CombinedModel(model,rgbmodel,fusionmodel)
    ...
    if args.finetune:
        ...
        state_dict = combinedModel.state_dict()
        ...
        if "model.pos_embed" in checkpoint_model:
            pos_embed_checkpoint = checkpoint_model["model.pos_embed"]
            embedding_size = pos_embed_checkpoint.shape[-1]  # channel dim
            num_patches = combinedModel.model.patch_embed.num_patches
            num_extra_tokens = combinedModel.model.pos_embed.shape[-2] - num_patches  # 0/1
        ...
        if 'model.time_embed' in checkpoint_model and args.num_frames != checkpoint_model['model.time_embed'].size(1):
            time_embed = checkpoint_model['model.time_embed'].transpose(1, 2).float()
            new_time_embed = F.interpolate(time_embed, size=(args.num_frames), mode='nearest')
            checkpoint_model['model.time_embed'] = new_time_embed.transpose(1, 2)

        utils.load_state_dict(combinedModel, checkpoint_model, prefix=args.model_prefix)
    ...
    rgbmodel_without_ddp = rgbmodel
    combinedModel_without_ddp = combinedModel
    ...
    num_layers = model_without_ddp.get_num_layers()+rgbmodel_without_ddp.get_num_layers()
    ...
    combined_skip_weight_decay_list = combinedModel.no_weight_decay()
    #rgb_skip_weight_decay_list = rgbmodel.no_weight_decay()
    print("Skip weight decay list: ", combined_skip_weight_decay_list)
    ...
    if args.enable_deepspeed:
        ...
        rgboptimizer_params = get_parameter_groups(
            rgbmodel,
            args.weight_decay,
            [],
            assigner.get_layer_id if assigner is not None else None,
        ...
        rgbmodel, rgboptimizer, _, _ = ds_init(
            args=args,
            model=rgbmodel,
            model_parameters=rgboptimizer_params,
            dist_init_required=not args.distributed,
        )
        '''

        combinedoptimizer_params = get_parameter_groups(
            combinedModel,
            args.weight_decay,
            combined_skip_weight_decay_list,
            assigner.get_layer_id if assigner is not None else None,
            assigner.get_scale if assigner is not None else None,
        )
        combinedModel, combinedoptimizer, _, _ = ds_init(
            args=args,
            model=combinedModel,
            model_parameters=combinedoptimizer_params,
            dist_init_required=not args.distributed,
        )

        print(
            "model.gradient_accumulation_steps() = %d"
            % combinedModel.gradient_accumulation_steps()
        )
        assert combinedModel.gradient_accumulation_steps() == args.update_freq
    else:
        if args.distributed:
    ...
    utils.auto_load_model(
        args=args,
        model=rgbmodel,
        model_without_ddp=rgbmodel_without_ddp,
        optimizer=rgboptimizer,
        loss_scaler=loss_scaler,
        model_ema=None,
    )
    '''
    ...
    utils.auto_load_model(
        args=args,
        model=combinedModel,
        model_without_ddp=combinedModel_without_ddp,
    ...
    if args.eval:
        ...
        search_list = np.linspace(0, 1, 11)
        test_stats = final_phase_test(data_loader_test, combinedModel,search_list, device, preds_file)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            test_stats = validation_one_epoch(data_loader_val,combinedModel,device)
            print(
