...
class PhaseDataset_Cholec80(Dataset):
    ...
    def __getitem__(self, index):
        if self.mode == "train":
            ...
            video_id, frame_id, frames,instrument_labels = (
        ...
        for video_id in infos.keys():
            ...
            for line_info in data:
                ...
                img_path = os.path.join(
                    self.data_path,
                    "cutMargin",
                    str(int(line_info["video_id"].replace('video',''))),
                    str(int((line_info["frame_id"])+1)) + ".png",
                )
    ...
    def _video_batch_loader(self, duration, indice, video_id, index, cut_black):
        ...
        currentFramePath = self.dataset_samples[index]["img_path"]
        currentFrameData = np.array(Image.open(currentFramePath))
        if np.all(currentFrameData == video_data[-1])==False:
            print("当前帧挑错了")
