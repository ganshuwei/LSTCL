...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)  # (B, KT+1, C)
        return x  # 返回完整特征，包括cls token和所有patch tokens

    def forward(self, x):
        features = self.forward_features(x)  # (B, KT+1, C)
        cls_token = features[:, 0]  # 取出cls token用于分类
        output = self.head(self.fc_dropout(cls_token))
        return output, features  # 返回分类输出和完整特征
